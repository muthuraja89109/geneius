...
export default function ChatPage() {
  ...
  // If we arrived here from the homepage drag-and-drop, auto-send that file once.
  useEffect(() => {
    const pendingFile = location.state?.pendingFile;
    if (pendingFile && !autoSendHandled.current) {
      autoSendHandled.current = true;
      handleSend({
        text: location.state?.pendingText || "Here's a screenshot of a website — take a look.",
        file: pendingFile,
        previewUrl: location.state?.pendingPreviewUrl || null,
      });
      // clear the router state so refreshing /chat doesn't resend
